function Cart(props) {
  ...
  React.useEffect(() => {
    if(window.localStorage.getItem('jwtToken')){
      api.getProfile(window.localStorage.getItem('jwtToken')).then((json) => {setProfile(json.data)
  ...
  function checkout() {
    ...
    if(subtotal + freight -parseInt(sub)<0){
      window.alert('訂單金額錯誤')
      return;
    }
        ...
        .checkout(
          {
            ...
            order: {
              ...
              subtotal,
              freight,
              total: subtotal + freight -parseInt(sub),
              recipient,
  ...
  function changeTotal(){
    ...
    const str=document.querySelector('#dList').value;
    setSub(str.split(" ")[2])
    setDId((str.split(" ")[0]).substring(1))

    if(isNaN(str.split(" ")[2])===true){
      document.querySelector('#payable_total').innerText=subtotal + freight ;
      document.querySelector('.deduct').innerHTML=''
    }else{
      document.querySelector('#payable_total').innerText=subtotal + freight -parseInt(str.split(" ")[2]);
      document.querySelector('.deduct').innerHTML=` <div className="deduct__name">折扣</div>\
                                                    <div className="total__nt">NT.</div>\
                                                    <div className="total__amount">${str.split(" ")[2]}</div>`
    }
  ...
  return (
    <div className="cart">
      ...
      <div className="discount">
        <hr styles={{backgroundColor:'black'}}/>
        <br/>
        <font>折扣優惠券</font>
        <select id='dList' type='text' list="disList" onClick={inputDiscount}  onChange={changeTotal} >
          <option>請選擇優惠券</option>
          {discount.map((x)=><option className='discountsOpt'>#{x.discountNumber}&ensp;{x.cate}&ensp;{x.substract}&ensp;元</option>)}
        </select>
      </div>
      <div className="total">
      ...
      <div className="payable">
        ...
        <div className="total__nt">NT.</div>
        <div className="total__amount" id="payable_total">{subtotal + freight}</div>
      </div>
