...
const payOrderByPrime = async function(tappayKey, tappayId, prime, order){
    ...
    const [result] = await pool.query('UPDATE user SET spend = spend + ? WHERE email = ?', [order.total, order.recipient.email]);
    return res.body;
};
...
const delDisNum = async (discountNumber)=>{
    if(discountNumber!==0){
        await pool.query('UPDATE discount SET times=(SELECT times)-1 WHERE discountNumber=?',discountNumber)
        const [restNum]=await pool.query('SELECT times FROM discount WHERE discountNumber=?',discountNumber)
    if(restNum[0].times==0){
        await pool.query('DELETE FROM discount WHERE discountNumber = ?',discountNumber)
    }
    }
}
