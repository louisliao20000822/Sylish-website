...
const getCampaigns = async (req, res) => {
    ...
    campaigns.map((campaign) => {
        const imagePath = util.getImagePath(campaign.product_id);
...
const addsale = async (req, res) => {
    const Time = new Date().getTime() + parseInt(req.body.time*1000*60);
    console.log(Time);
    const detail = {
        did: req.body.did,
        cate: req.body.cate,
        substract: req.body.substract,
        time:  Time
    };
    const saleId = await Marketing.addsale(detail);
    res.send([{saleId}]);
};

const getsale = async (req, res) => {

    const result = await Marketing.getsale();
